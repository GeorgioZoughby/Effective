...
cudaError_t cuda_matrix_compare_equality(const T* elts1, const T* elts2, unsigned int size, bool* equality) {
    ...
    *equality = true; // Initialize to true
    ...
    cudaStatus = cudaMemcpyAsync(dev_equality, equality, sizeof(bool), cudaMemcpyHostToDevice, stream);
    if (cudaStatus != cudaSuccess) goto Error;
...
cudaError_t cuda_matrix_dot(const T* elts1, const T* elts2, unsigned int size, T* result) {
    ...
    *result = T(); // Initialize to zero
    ...
    cudaStatus = cudaMemcpyAsync(dev_result, result, sizeof(T), cudaMemcpyHostToDevice, stream);
    if (cudaStatus != cudaSuccess) goto Error;
...
cudaError_t cuda_matrix_multiply(const T* A, const T* B, T* C, int rowsA, int colsA, int colsB) {
    ...
    cudaStatus = cudaMemcpyAsync(dev_A, A, rowsA * colsA * sizeof(T), cudaMemcpyHostToDevice, stream);
    if (cudaStatus != cudaSuccess) goto Error;

    cudaStatus = cudaMemcpyAsync(dev_B, B, colsA * colsB * sizeof(T), cudaMemcpyHostToDevice, stream);
    if (cudaStatus != cudaSuccess) goto Error;
    ...
    cudaStatus = cudaMemcpyAsync(C, dev_C, rowsA * colsB * sizeof(T), cudaMemcpyDeviceToHost, stream);
    if (cudaStatus != cudaSuccess) goto Error;
...
cudaError_t cuda_matrix_transpose(const T* input, T* output, int rows, int cols) {
    ...
    cudaStatus = cudaMemcpyAsync(dev_input, input, rows * cols * sizeof(T), cudaMemcpyHostToDevice, stream);
    if (cudaStatus != cudaSuccess) goto Error;
    ...
    cudaStatus = cudaMemcpyAsync(output, dev_output, rows * cols * sizeof(T), cudaMemcpyDeviceToHost, stream);
    if (cudaStatus != cudaSuccess) goto Error;
